...
- 🟡 把 Tag 更名互動對齊商家更名：預設只顯示預覽按鈕、預覽後才出現黃色預覽匡與確認按鈕、確認送出中加旋轉 icon，並把錯誤訊息卡樣式（移除 ⚠️ icon）以及更名完成後切到新 tag 編輯畫面的行為一併對齊。（計劃：[tag-rename-confirm-alignment.md](docs/todo-references/tag-rename-confirm-alignment.md)）
