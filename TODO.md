...
7. 🔴 複製或新增交易後，首頁應跳到新記錄的日期，避免資料已建立但畫面仍停在原日期。
8. 🔴 修正今天按鈕的光暈上方被截斷的視覺問題。
9. 🔴 修正設定頁面 header 顏色不符合整體視覺設計的問題。
10. ✅ 統計卡片顯示多幣別時已改用統一圖示表示，不使用文字。
11. 🔴 提供可手動從 cloud 拉資料回本機的功能，作為現有同步流程之外的明確操作入口。
12. 🔴 將 `AddTransactionModal` 中阻擋式的 `alert()` 驗證改為 modal 內的內嵌錯誤提示。
13. 🔴 將 `AddTransactionModal` 中刪除用的 `confirm()` 改為 app 內建的確認 UI，並評估以 `swal2` 統一 alert / confirm 互動風格。
14. 🟡 將交易項目的支付方式改為可用圖示顯示，提升列表辨識度。
15. 🟡 將目前已存在的 `Gemini` 相關功能打開並接到可用入口，補齊必要的設定、錯誤提示與使用者回饋。
16. 🟡 新增依類別彙整的統計頁或統計區塊，提供金額與筆數等基礎分析。
17. 🟡 新增依商家彙整的統計頁或統計區塊，方便查看常去商家與消費分布。
18. 🟡 評估是否將 `SettingsPage` 內更多 section 進一步升級為獨立 page，讓偏好設定、同步設定、Tag 管理、匯入匯出與危險操作可各自擁有更完整的資訊架構。
19. 🟢 評估為非首頁頁面引入共用 page-shell pattern，讓 layout chrome 維持一致，同時讓 `App.tsx` 持續聚焦於 routing 與 shared state。
...
- 🔴 複製之後沒有跳到新記錄當天，新增完畢後首頁應該要跳到新記錄的日期。
- 🔴 修正今天按鈕的光暈上方被截斷的視覺問題。（示意圖：[today-button-glow-clipped.jpg](docs/todo-references/today-button-glow-clipped.jpg)）
- 🟡 將交易項目的支付方式改為可用圖示顯示，提升列表辨識度。（示意圖：[payment-method-icon.jpg](docs/todo-references/payment-method-icon.jpg)）

## 設定頁體驗

- 🔴 修正設定頁面 header 顏色不符合整體視覺設計的問題。（示意圖：[settings-header-color.jpg](docs/todo-references/settings-header-color.jpg)）
