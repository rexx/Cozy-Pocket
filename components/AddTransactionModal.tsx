...
const AddTransactionModal: React.FC<AddTransactionModalProps> = ({ 
  ...
  const addTag = () => {
    ...
    if (input) {
      ...
      // 無論是否新增了標籤，只要有輸入內容，按下確認鍵後就清空輸入框，避免卡頓感
      setTagInput('');
    }
  };
  ...
  const handleTagKeyDown = (e: React.KeyboardEvent) => {
    // 空白鍵不再觸發送出 (e.key === ' ')，使用者可以在輸入框內自由打空格
  ...
  const handleSubmit = () => {
    ...
    if (input) {
      const parts = input.split(/\s+/).map(p => p.replace(/#/g, ''));
      parts.forEach(part => {
        if (part && !finalTagList.includes(part)) {
          finalTagList.push(part);
        }
      });
    }
  ...
  const currentMainCat = useMemo(() => EXPENSE_CATEGORIES.find(c => c.id === categoryId), [categoryId]);
