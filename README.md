...
*   `統計`：分為「總覽」與「趨勢」兩個分頁。總覽提供月份／年份切換，tag、支付方式與類別篩選，收入／支出明細展開，依類別彙整以及依商家彙整的金額與筆數分析；趨勢以近 12 個月滾動視窗的堆疊長條圖呈現每月收支組成，並可依類別篩選（見 §6.14）。
...
*   篩選面板由上而下為 `TAG`、`支付方式`、`類別` 三列 chip。類別篩選與 Tag 一樣支援多選，但採 OR 規則：交易只要屬於任一已選類別就納入統計。這與 Tag 的 AND 規則不同——一筆交易只有一個類別，AND 在此永遠得到空集合。
...
*   幣別卡片右上角的篩選徽章在只選一個類別時顯示類別名稱，選取多個時顯示「N 個類別」，與 Tag 的收合規則一致。徽章的組成順序為 tag、支付方式、類別、已排除子類別，與篩選面板由上而下的順序相同。
...
*   點選任一月份的長條會在圖下方顯示該月總額與前 5 大類別，其餘併為「其他 N 個類別」；預設聚焦視窗最後一個月，切換月份、幣別或收支類型時重設回最後一個月。
*   圖下方三格摘要為月均、最高月與較上月。金額使用與 Y 軸相同的精簡格式且不帶幣別符號（卡片標題已標明幣別），月份與對比基準放在各自的說明次行；三格在 iPhone 寬度下每格只有約 74px 文字空間，把月份與金額擠在同一行必定截斷。
*   底部的類別 chip 依近 12 個月總額由大到小排列並附各自總額，點一下把該類別移出堆疊、再點一下加回。這排 chip 同時是堆疊圖的圖例，排序與堆疊順序一致。
*   底部 chip 與篩選面板的類別篩選作用層級不同：chip 只決定這張圖要畫哪幾條色帶，狀態隨收支類型切換而重設、離開趨勢分頁即消失；篩選面板改變的是整頁的資料集，跨分頁保留，也影響總覽的彙整與明細。兩者疊加時篩選先生效，chip 只能在剩下的類別裡挑。
*   chip 區右上提供「全選」與「取消全選」，依目前狀態出現：全部顯示時只有「取消全選」，全部取消時只有「全選」，部分顯示時兩個都在。因此只看單一類別是兩下（取消全選 → 點該類別），不必逐一關掉其他類別。
*   全部取消時圖區顯示「沒有選取任何類別」，且月份摘要與三格數字卡一併收起，不顯示沒有意義的 0。
*   空狀態文案先判斷「有沒有資料」再判斷「是不是全部取消」。該幣別在視窗內完全沒有該類型的紀錄時顯示「近 12 個月沒有<類型>紀錄」，即使此時可見類別清單同樣是空的也不會誤報成「沒有選取任何類別」。套用類別篩選後很容易踩到這個情境：篩掉全部支出類別再切到收入分頁即是。
