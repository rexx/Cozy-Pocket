...
*   `統計`：分為「總覽」與「趨勢」兩個分頁。總覽提供月份／年份切換、tag 與支付方式篩選、收入／支出明細展開、依類別彙整以及依商家彙整的金額與筆數分析；趨勢以近 12 個月滾動視窗的堆疊長條圖呈現每月收支組成，並可依類別篩選（見 §6.14）。
...
*   tag、支付方式與已排除子類別三個篩選條件由總覽與趨勢共用同一份 state，切換分頁不會重置，幣別卡片右上的篩選徽章在兩個分頁都照常顯示。
*   可選的 tag 與支付方式清單取自目前分頁的實際範圍：總覽取選定月份或年份，趨勢取 12 個月的滾動視窗，因此在趨勢下可以選到幾個月前才用過的 tag。
...
*   底部的類別 chip 依近 12 個月總額由大到小排列並附各自總額，點一下把該類別移出堆疊、再點一下加回。
*   chip 區右上提供「全選」與「取消全選」，依目前狀態出現：全部顯示時只有「取消全選」，全部取消時只有「全選」，部分顯示時兩個都在。因此只看單一類別是兩下（取消全選 → 點該類別），不必逐一關掉其他類別。
*   全部取消時圖區顯示「沒有選取任何類別」，且月份摘要與三格數字卡一併收起，不顯示沒有意義的 0。該幣別在視窗內完全沒有該類型的紀錄時，顯示「近 12 個月沒有<類型>紀錄」。
