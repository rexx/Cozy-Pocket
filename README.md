...
*   `資料與設定`：提供偏好設定、同步設定、Tag 管理、匯入匯出與危險操作等單頁分區。
*   `同步狀態`：顯示待同步／同步中／已同步／失敗統計、交易清單、預設隱藏已同步項目的篩選、詳細錯誤與手動同步入口。
...
*   `資料與設定` 目前維持單一頁面，但已拆成偏好設定、同步設定、Tag 管理、匯入匯出與危險操作等區段元件。
...
*   畫面已拆成五個區段元件：`PreferencesSection`、`SyncSection`、`TagManagementSection`、`ImportExportSection`、`DangerZoneSection`。
