...
*   `統計`：提供月份／年份切換、tag 與支付方式篩選、收入／支出明細展開，以及依類別彙整的金額與筆數分析。
...
*   `新增／編輯交易`：唯一保留的 overlay，會覆蓋在目前主頁面上，關閉後返回原頁；編輯模式底部會顯示單筆同步狀態與可用的單筆上傳操作。
...
*   期間切換會同步影響幣別統計、Tag 清單、支付方式清單、收入／支出展開明細與類別彙整結果。
...
6. 在「依類別彙整」中點擊類別列，查看子類別摘要與該類別交易列表
7. 點擊列表項目可直接編輯該筆交易
