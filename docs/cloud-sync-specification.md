...
8. 交易編輯頁單筆上傳（Manual Single Item Retry from Edit Modal）
- 觸發：使用者在交易編輯頁底部的同步狀態區塊點擊左側狀態圖示。
- 適用狀態：`pending` 的時鐘圖示可立即上傳；`error` 的驚嘆號圖示可重新上傳。
- 行為：前端讀取 IndexedDB 中該筆交易的已儲存版本，呼叫既有 `create` upsert 流程同步單筆交易，完成後重新載入本地資料。
- 離線、同步中、找不到本地交易或未設定 Sync API URL / Token 時，圖示按鈕應停用，並在狀態區塊顯示對應說明。
- 若表單內容已被修改但尚未儲存，單筆上傳仍以 IndexedDB 中已儲存版本為準。
...
- 交易編輯頁底部會顯示該筆交易的同步狀態：
  - `pending`：顯示時鐘圖示與「待同步」，左側圖示可點擊立即上傳。
  - `syncing`：顯示轉動圖示與「同步中」，不可手動觸發。
  - `synced`：顯示勾選圖示與「已同步」，不可手動觸發。
  - `error`：顯示驚嘆號圖示與「同步失敗」，左側圖示可點擊重新上傳；若有 `lastSyncError`，會顯示錯誤摘要。
- 編輯頁同步狀態使用獨立同步資訊更新，不以背景同步結果重建整筆表單資料，避免覆蓋尚未儲存的欄位。
...
- 前端目前有 5 種通知 / 狀態呈現機制：
  ...
  - 全域 toast：底部輕量、短暫、非阻擋、單行摘要；用於交易新增／修改／刪除成功、連線狀態切換，以及設定頁操作「全部成功」時的摘要。
  - 頁內 status message：顯示較長結果，可保留換行與細節；只在離線待同步、部分同步失敗、需要後續行動（例如「查看同步狀態」按鈕）或發生錯誤時顯示。
  - 同步狀態頁：持續型狀態檢視，顯示統計與單筆錯誤詳情。
  - 全域錯誤面板：偏除錯用途，顯示系統層級錯誤與同步診斷。
- 使用原則：
  - `sweetalert2` confirmation dialog 只用於需要使用者明確選擇「繼續 / 取消」的流程；交易新增、修改、刪除成功已改用全域 toast，不再使用 `sweetalert2` 置中 auto-dismiss toast。
  - toast 與頁內 status 互斥：同一次操作只會呈現其中一種，不會同時出現。
  - 全部成功時只顯示底部 toast，不顯示頁內 status card。
  - 離線待同步、部分同步失敗、需要後續行動或發生錯誤時，只顯示頁內 status，不額外跳 toast。
  - 同步狀態頁提供可回看、可追查的詳細狀態，不用 toast 取代。
  - 表單驗證錯誤應在頁內或 modal 內嵌呈現，不使用 blocking `alert()`。
  - 全域錯誤面板不作為一般使用者成功 / 失敗提示的主要 UI。
- 匯入、同步設定、範例資料、匯出、Tag／商家更名等設定頁操作，應遵守以下規則：
  - 純成功：只顯示底部 toast，例如「匯入成功 (20 筆)」、「已將 #舊名 更名為 #新名，共更新 12 筆」。
  - 離線待同步、部分同步失敗或需要後續行動：只顯示頁內 status（可附「查看同步狀態」按鈕），不額外顯示 toast。
  - 完全失敗：頁內 status 顯示主要錯誤；若有系統層級細節，再同步寫入錯誤面板或同步狀態頁。
- 文案規則：
  - toast 文案應控制在短句，避免多個子句與過長數據細節。
  - 頁內 status 可用多行文案，第一行先給結果摘要，第二行再補充同步或離線資訊。
  - 同一操作的 toast 與頁內 status 不會同時出現；若某個場景意外同時顯示兩者，視為需要修正的 bug。
